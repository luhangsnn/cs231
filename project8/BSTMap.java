...
public class BSTMap<K, V> implements MapSet<K, V> {
	...
	// return the getDepth method in TNode class
	public int getDepth(){
		if (this.root == null){
			return 0;
		}
	...
	// traverse in order and add each element into a heap
	public void addToHeap(PQHeap <KeyValuePair<K,V>> h) {
		if (this.root != null) {
			this.root.addToHeap(h);
		}
	}
    ...
    private class TNode {
		...
		public void addToHeap (PQHeap <KeyValuePair<K, V>> h) {
			if (this.left != null){
				this.left.addToHeap(h);
			}
			h.add(this.dataPair);
			// System.out.println("added: " + this.dataPair);
			if (this.right != null){
				this.right.addToHeap(h);
			}
		}
